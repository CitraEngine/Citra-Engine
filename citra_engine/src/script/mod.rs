...
pub trait CitraBehaviour {
    ...
    fn on_load(&self) -> Result<(), CitraError> {
        Ok(())
    }
    fn on_start(&self) -> Result<(), CitraError> {
        Ok(())
    }
    fn on_enable(
        &self,
    ...
    fn on_destroy(&self) -> Result<(), CitraError> {
        Ok(())
    }
}
...
impl ScriptVariant {
    ...
    pub fn get_name(&self) -> String {
        self.script.borrow().get_name()
    }
    pub fn on_load(&self) -> Result<(), CitraError> {
        self.script.borrow().on_load()
    }
    pub fn on_start(
        &mut self,
        obj: Rc<RefCell<GameObject>>,
        ...
        self.script.borrow().on_start()?;
        let mut obj_mut = obj.borrow_mut();
        let enabled = obj_mut.is_enabled();
        drop(obj_mut);
        if enabled {
            self.script.borrow().on_enable(obj, ctx, &mut self.args)?;
        }
        Ok(())
    }
    pub fn on_enable(
    ...
    pub fn on_tick(
        ...
        self.script.borrow().on_tick(obj, ctx, &mut self.args)
    }
    pub fn on_disable(
        &mut self,
        obj: Rc<RefCell<GameObject>>,
        ...
        self.script.borrow().on_disable(obj, ctx, &mut self.args)
    }
    pub fn on_destroy(
        &mut self,
        obj: Rc<RefCell<GameObject>>,
...
fn serialize_citra_behaviour_rc<S>(
    behaviour: &Rc<RefCell<dyn CitraBehaviour>>,
    serializer: S,
) -> Result<S::Ok, S::Error>
where
    S: Serializer,
...
fn deserialize_citra_behaviour_rc<'de, D>(
    deserializer: D,
) -> Result<Rc<RefCell<dyn CitraBehaviour>>, D::Error>
where
    D: Deserializer<'de>,
{
    ...
    Engine::get_scripts()
        ...
        .ok_or_else(|| de::Error::custom(format!("There is no script named '{}'", name)))
}
